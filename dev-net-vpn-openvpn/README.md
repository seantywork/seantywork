...
```shell
sudo apt update
sudo apt install libnl-genl-3-dev libcap-ng-dev liblz4-dev liblzo2-dev libpam0g-dev
...
ca /etc/openvpn/chain.cert.pem
cert /etc/openvpn/x509/server.cert.pem
key /etc/openvpn/server.key.pem  # This file should be kept secret
...
push "route 172.168.101.0 255.255.255.0"
...
sudo ip link add dev oeth1 type veth peer name oeth2 netns v1

sudo ip netns exec v1 ip link set oeth2 up

sudo ip netns exec v1 ip addr add 172.168.101.2/24 dev oeth2

sudo ip netns exec v1 ip route add default via 172.168.101.1 dev oeth2

sudo ip netns exec v1 ip link set up lo

sudo ip addr add 172.168.101.1/24 dev oeth1

sudo ip link set up oeth1
