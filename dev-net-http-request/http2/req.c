...
static int setup(struct transfer *t, int num)
{
    ...
    if(curl) {
        ...
        /* set the same URL */
        curl_easy_setopt(curl, CURLOPT_URL, this_url);

        curl_easy_setopt(curl, CURLOPT_CAINFO, this_ca);
        curl_easy_setopt(curl, CURLOPT_SSLCERT, this_cert);
        curl_easy_setopt(curl, CURLOPT_SSLKEY, this_key);
